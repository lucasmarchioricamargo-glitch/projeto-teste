  ...
  coderabbit_integration:
    ...
    workflow: |
      ...
      WHILE iteration < max_iterations:
        1. Run: cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --prompt-only --base main
    ...
    commands:
      qa_pre_review_uncommitted: "cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --prompt-only -t uncommitted"
      qa_story_review_committed: "cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --prompt-only --base main"
    execution_guidelines: |
      NOTE: Environment is WSL2/Linux — run commands directly, no wrapper needed.

      **How to Execute:**
      1. Run commands directly (already inside WSL2 — no 'wsl bash -c' wrapper)
      2. Always include the 'review' subcommand: coderabbit review [options]
      ...
      **Error Handling:**
      - If "coderabbit: command not found" → verify ~/.local/bin/coderabbit exists
      - If timeout → increase timeout, review is still processing
      - If "not authenticated" → run: ~/.local/bin/coderabbit auth status
