...
## Execução (WSL2/Linux — sem wrapper)

```bash
# Self-healing mode (automatic in dev tasks)
cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --severity CRITICAL,HIGH --auto-fix

# Manual review
cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review -t uncommitted

# Prompt-only mode
cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --prompt-only -t uncommitted

# Committed changes vs base branch (PR review)
cd ${PROJECT_ROOT} && ~/.local/bin/coderabbit review --prompt-only --base main
```
